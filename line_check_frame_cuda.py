...
def line_check(frame, M, Minv, LT, reusable_gpu_img=None):
    orig = frame.copy()
    """
    ...
    if reusable_gpu_img is not None:
        color = line_check_Preprocessing_cuda(orig, threshold_val, M, reusable_gpu_img)
    else:
...
def line_check_sobel(frame, M, Minv, LT, reusable_gpu_img=None):
    orig = frame.copy()
    ...
    """
    if reusable_gpu_img is not None:
        color = line_check_sobel_Preprocessing_cuda(orig, M, reusable_gpu_img)
    else:
...
#여기서부터 주요 예시
#video_file = 'project'
def combined_threshold_cuda(img, reusable_gpu_img):
    # ▶ 이미지 GPU 업로드
    reusable_gpu_img.upload(img)

    # ▶ 그레이 변환
    gpu_gray = cv.cuda.cvtColor(reusable_gpu_img, cv.COLOR_BGR2GRAY)

    # ▶ Sobel X + abs + 8U 변환
    gpu_sobelx = cv.cuda.Sobel(gpu_gray, cv.CV_32F, 1, 0, ksize=3)
    gpu_sobel_8u = cv.cuda.convertScaleAbs(gpu_sobelx)
    _, sxbinary = cv.cuda.threshold(gpu_sobel_8u, 15, 255, cv.THRESH_BINARY)

    gpu_hls = cv.cuda.cvtColor(reusable_gpu_img, cv.COLOR_BGR2HLS)

    # S 채널 범위 마스크 생성 (S 채널은 HLS의 3번째 채널)
    # -> 즉, HLS 전체에서 S 채널 값이 90~255인 픽셀을 선택
    lower_bound = (0, 0, 90)
    upper_bound = (180, 255, 255)

    # inRange로 바로 마스크 생성
    s_binary = cv.cuda.inRange(gpu_hls, lower_bound, upper_bound)


    # ▶ OR 연산 결합
    gpu_combined = cv.cuda.bitwise_or(sxbinary, s_binary)

    return gpu_combined

def open_img_cuda(gpu_img, iterations):


    morph_filter = cv.cuda.createMorphologyFilter(op=cv.MORPH_OPEN, 
                                            srcType=cv.CV_8UC1, 
                                            kernel=kernel_small,
...
def warp_cuda(gpu_img, M):
    M_gpu = np.asarray(M, dtype=np.float32)
    width, height = gpu_img.size()
    warped = cv.cuda.warpPerspective(gpu_img, M_gpu, (width, height), flags=cv.INTER_NEAREST)
    return warped

def color_space_hls_cuda(img, reusable_gpu_img):
    # ▶ GPU 업로드
    reusable_gpu_img.upload(img)

    # ▶ HLS 및 HSV 변환
    gpu_hls = cv.cuda.cvtColor(reusable_gpu_img, cv.COLOR_BGR2HLS)
    gpu_hsv = cv.cuda.cvtColor(reusable_gpu_img, cv.COLOR_BGR2HSV)

    # ▶ 마스크 생성
    # 노랑 마스크 (HLS 기준)
    lower_yellow = np.array([20, 0, 0], dtype=np.uint8)
    upper_yellow = np.array([27, 255, 255], dtype=np.uint8)
    mask_yellow = cv.cuda.inRange(gpu_hls, lower_yellow, upper_yellow)

    # 흰색 마스크 (HLS 기준)
    lower_white = np.array([0, 0, 200], dtype=np.uint8)
    upper_white = np.array([180, 255, 255], dtype=np.uint8)
    mask_white = cv.cuda.inRange(gpu_hls, lower_white, upper_white)

    # 초록 마스크 (HSV 기준)
    lower_green = np.array([35, 50, 50], dtype=np.uint8)
    upper_green = np.array([85, 255, 255], dtype=np.uint8)
    green_mask = cv.cuda.inRange(gpu_hsv, lower_green, upper_green)

    # ▶ 노랑 - 초록 제거 (bitwise_not & and)
    not_green = cv.cuda.bitwise_not(green_mask)
    yellow_clean = cv.cuda.bitwise_and(mask_yellow, not_green)

    # ▶ 흰색 + 노랑 결합
    combined_mask = cv.cuda.bitwise_or(yellow_clean, mask_white)

    # ▶ 마스크 적용
    masked_gpu = cv.cuda.bitwise_and(reusable_gpu_img, reusable_gpu_img, mask=combined_mask)

    # ▶ 그레이 변환
    gray_gpu = cv.cuda.cvtColor(masked_gpu, cv.COLOR_BGR2GRAY)

    return gray_gpu  # 또는 gray_gpu.download() 로 CPU 이미지 반환

def line_check_sobel_Preprocessing_cuda(img, M, reusable_gpu_img):
    gpu_combined = combined_threshold_cuda(img, reusable_gpu_img)
    binary_result = open_img_cuda(gpu_combined, 1)
    color_gpu = warp_cuda(binary_result, M)
    color = color_gpu.download()
    return color

def line_check_Preprocessing_cuda(img, threshold_val, M, reusable_gpu_img):
    gray_gpu = color_space_hls_cuda(img, reusable_gpu_img)
    _, binary_result = cv.cuda.threshold(gray_gpu, threshold_val, 255, cv.THRESH_BINARY)
    color_gpu = warp_cuda(binary_result, M)
    color = color_gpu.download()
    return color
...
def example():
    ...
    if USE_CUDA == True:
        reusable_gpu_img = cv.cuda_GpuMat()
    else:
        reusable_gpu_img = None
    ...
    while(cap.isOpened()):
        ...
        frame = line_check_sobel(frame, M, Minv, LT, reusable_gpu_img)
